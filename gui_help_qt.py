...
def show_print_bidders_help(gui):
    """Show help dialog for printing bidders."""
    dialog = QDialog(gui)
    dialog.setWindowTitle("SwiftSale Help")
    dialog.setFixedSize(600, 400)
    layout = QVBoxLayout(dialog)
    title_label = QLabel("Printing Bidders (SwiftSale)")
    title_label.setFont(QFont("Arial", 12, QFont.Bold))
    title_label.setWordWrap(True)
    title_label.setAlignment(Qt.AlignCenter)
    layout.addWidget(title_label)
    help_text = (
        "The 'Print Bidders' button refreshes the bidders table with the current list of bidders and their assigned bins.\n\n"
        "**Steps**:\n"
        "1. Click 'Print Bidders' to update the table.\n"
        "2. The table shows Username, Quantity, Bin, Giveaway status, Weight, and Timestamp.\n"
        "3. Use 'Sort by Bin' buttons to reorder the table.\n\n"
        "**Notes**:\n"
        "- Ensure bidders are added via the 'Add Bidder' section.\n"
        "- The table updates automatically when new bidders are added."
    )
    help_content = QTextBrowser()
    help_content.setPlainText(help_text)
    layout.addWidget(help_content)
    ...
    layout.addWidget(close_button)
    dialog.exec()
    gui.log_info("Displayed print bidders help pop-up")
