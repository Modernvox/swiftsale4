...
- Stripe API/webhook keys removed — only STRIPE_PUBLIC_KEY remains for UI if desired.
"""
...
def save_install_info(email: str, install_id: str, tier: str, promo_expiration: Optional[datetime] = None) -> None:
    try:
...
def get_or_create_install_info() -> dict:
    ...
    if "promo_expiration" not in info:
        ...
        changed = True
    if changed:
        save_install_info(info["email"], info["install_id"], info["tier"], promo_expiration=info.get("promo_expiration"))
    return info
...
def load_config() -> dict:
    ...
    config: dict = {
        ...
        # Stripe public key only (optional UI use)
        "STRIPE_PUBLIC_KEY": os.getenv("STRIPE_PUBLIC_KEY", ""),
        "SUCCESS_URL": f"{os.getenv('APP_BASE_URL', 'http://localhost:5000')}/success",
        "CANCEL_URL": f"{os.getenv('APP_BASE_URL', 'http://localhost:5000')}/cancel",
    }

    # Server mode strict checks
    if _is_server_runtime():
        required = ["SECRET_KEY", "APP_BASE_URL", "DATABASE_URL"]
...
__all__ = [
    "PRICE_MAP", "REVERSE_PRICE_MAP", "TIER_LIMITS",
    "DEFAULT_DATA_DIR", "DEFAULT_UPDATE_MANIFEST_URL",
    "load_config", "save_config", "get_config_value", "reload_config_cache",
    "load_install_info", "save_install_info", "get_or_create_install_info",
    "get_resource_path", "get_update_manifest_url", "parse_bool_env",
    "sanitize_username", "AUTO_CAPTURE_USERNAMES_DEFAULT",
]
