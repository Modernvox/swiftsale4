...
from config_qt import get_resource_path, load_config, DEFAULT_DATA_DIR, TIER_LIMITS, load_install_info, save_install_info
...
from gui_help_qt import (
    show_giveaway_help, show_telegram_help, show_print_bidders_help, show_import_csv_help,
...
from gui_events import bind_event_methods
...
class SwiftSaleGUI(QMainWindow):
    def __init__(self, stripe_service, api_token, user_email, base_url, dev_unlock_code, telegram_bot_token, telegram_chat_id, dev_access_granted, log_info, log_error, bidder_manager, bidders_db_path, subs_db_path):
        ...
        self.default_x_offset_in = 2.3
        self.default_y_offset_in = 5.6
        ...
        if config['FLASK_ENV'] == 'production':
            try:
                ...
                if cloud_install:
                    ...
                    self.log_info(f"Synced install from cloud: email={self.user_email}, install_id={self.install_id}, tier={self.tier}")
                else:
                    self.log_info(f"No cloud install found for {self.user_email}; using local data")
            except Exception as e:
                self.log_error(f"Failed to sync with cloud database: {e}")
                self.cloud_db = None
        ...
        try:
            ...
            if self.tier != install_config['tier']:
                hashed_email = hashlib.sha256(self.user_email.encode()).hexdigest()
                self.bidder_manager.update_install(hashed_email, self.install_id, self.tier)
                save_install_info(self.user_email, self.install_id, self.tier)
                if self.cloud_db:
    ...
    def prompt_for_email(self):
        ...
        dialog.exec_()
        return email_input.text().strip() if '@' in email_input.text().strip() else ""
    ...
    def setup_connections(self):
        ...
        if hasattr(self, "print_bidders_button"):
            self.print_bidders_button.clicked.connect(self.populate_bidders_tree)
        ...
        if hasattr(self, "print_bidders_help_button"):
            self.print_bidders_help_button.clicked.connect(self.show_print_help)
        if hasattr(self, "import_csv_help_button"):
            self.import_csv_help_button.clicked.connect(self.show_import_help)
        if hasattr(self, "export_csv_help_button"):
            self.export_csv_help_button.clicked.connect(self.show_export_help)
        if hasattr(self, "sort_bin_asc_help_button"):
            self.sort_bin_asc_help_button.clicked.connect(self.show_sort_help)
        if hasattr(self, "sort_bin_desc_help_button"):
            self.sort_bin_desc_help_button.clicked.connect(self.show_sort_help)
        if hasattr(self, "clear_bidders_help_button"):
            self.clear_bidders_help_button.clicked.connect(self.show_clear_help)
    ...
    def build_subscription_ui(self, parent_frame):
        ...
        subscription_layout.addWidget(QLabel("Select Tier:"), 0, 0)
    ...
    def build_annotate_ui(self, parent_frame):
        ...
        annotate_button = QPushButton("Annotate 4\"×6\" Labels (SwiftSale App: Bin #)")
        annotate_button.clicked.connect(self.on_annotate_labels_clicked)
        annotate_layout.addWidget(annotate_button, 1, 0, 1, 6)
    ...
    def update_bins_used_display(self):
        """Update the bins used display."""
        try:
            bins_used = self.bidder_manager.count_user_bins(self.user_email)
    ...
    def populate_bidders_tree(self):
        """Populate the bidders_tree QTreeWidget."""
        try:
            ...
            for username, info in self.bidder_manager.bidders.items():
                parent = QTreeWidgetItem([
                    info["original_username"],
                    "", str(info["bin"]) if info["bin"] else "", "", "", ""
                ])
