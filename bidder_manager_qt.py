...
class BidderManager:
    ...
    def __init__(self, bidders_db_path, subs_db_path):
    ...
    def get_bin_count(self):
        """Return the total number of unique bins assigned."""
        try:
            cursor = self.conn.cursor()
    ...
    def count_user_bins(self, user_email):
    ...
    def add_transaction(self, username, original_username, qty, weight, is_giveaway):
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (uname, original_username, qty, weight, int(is_giveaway), bin_num, giveaway_num, timestamp, last_assigned))
            self.conn.commit()
