...
def setup_ui(main_window, is_dev_mode=None):
        ...
        main_window.setFixedWidth(900)  # Set window width
        ...
        # Expose header so other modules can inject widgets (e.g., chatbot button)
        main_window.header_container = header_frame
        main_window.header_layout = header_layout
        ...
        try:
                ...
                spacer = QSpacerItem(40, 0, QSizePolicy.Fixed, QSizePolicy.Minimum)
                header_layout.addItem(spacer)
        except Exception as e:
        ...
        main_window.header_label.setSizePolicy(QSizePolicy.Maximum, QSizePolicy.Preferred)
        header_layout.addWidget(main_window.header_label)

        # Stretch pushes action container to the far right
        header_layout.addStretch(1)

        # >>> This container is where the chat button will be inserted <<<
        # NOTE: Do NOT reference main_window.chat_btn here; it doesn't exist yet.
        main_window.header_action_container = QFrame()
        main_window.header_action_container.setObjectName("headerActionContainer")
        main_window.header_action_layout = QHBoxLayout(main_window.header_action_container)
        main_window.header_action_layout.setContentsMargins(0, 0, 0, 0)
        main_window.header_action_layout.setSpacing(2)
        # Keep a tiny spacer so future widgets have breathing room
        main_window.header_action_layout.addItem(QSpacerItem(2, 0, QSizePolicy.Fixed, QSizePolicy.Minimum))

        # Add the action container into the header (this was missing)
        header_layout.addWidget(main_window.header_action_container)
        ...
        bidders_header_frame = QFrame()
        bidders_header_layout = QHBoxLayout(bidders_header_frame)
        bidders_header_layout.addWidget(QLabel("Bidders"))
        main_window.toggle_button = QPushButton("+")
        main_window.toggle_button.setFixedWidth(25)
        bidders_header_layout.addWidget(main_window.toggle_button)
        bidders_header_layout.addStretch(1)
        bidders_layout.addWidget(bidders_header_frame)
        ...
        # ------------------------------------------------------------------
        # Browser Bridge panel inside Settings tab
        # ------------------------------------------------------------------
        try:
                ...
                # Wire up actions
                def on_enable_changed(_checked: bool):
                        try:
