...
def populate_bidders_tree(self):
    """Populate the bidders_tree QTreeWidget from bidder_manager's in-memory data."""
    try:
        self.bidders_tree.clear()
        ...
        for username, info in self.bidder_manager.bidders.items():
            ...
            # ✅ Collapse parent item correctly
            parent.setExpanded(False)

        self.bidders_tree.resizeColumnToContents(0)
        self.log_info("Updated bidders tree (collapsed by default)")
    except Exception as e:
...
def bind_bidders_methods(gui):
    ...
    gui.populate_bidders_tree = populate_bidders_tree.__get__(gui, gui.__class__)
